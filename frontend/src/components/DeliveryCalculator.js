...
export default function DeliveryCalculator({ address, onCostCalculated }) {
  ...
  const [displayCost, setDisplayCost] = useState(MIN_DELIVERY);
  ...
  // Анимация стоимости при изменении deliveryCost
  useEffect(() => {
    animateCost(displayCost, deliveryCost);
  ...
  return (
    <div className="delivery-calculator">
      ...
      {!loading && !error && distance === null && <p>Введите адрес для расчёта доставки</p>}
    </div>
  );
}
